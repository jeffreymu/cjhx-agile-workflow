...
test("agent messages are immutable-created without lost duplicate writes", (t) => {
  const storage = fixture(t); const value = message(); storage.createAgentMessage(value); assert.deepEqual(storage.getAgentMessage(value.id), value); assert.throws(() => storage.createAgentMessage({ ...value, body: "overwrite" }), /already exists/); assert.equal(storage.listAgentMessages()[0]?.body, value.body); assert.equal(lstatSync(resolve(storage.collaborationMessages, `${value.id}.json`)).mode & 0o777, 0o600);
});
