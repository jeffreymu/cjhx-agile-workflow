...
test("UI serves the control surface and workspace snapshot", async (t) => {
  ...
  const page = await fetch(base); assert.equal(page.status, 200); const html = await page.text(); assert.match(html, /CJHX Agile Workflow/); assert.match(html, />工作空间</); assert.match(html, /data-view="changes"[^>]*>.*>研发流程</); assert.doesNotMatch(html, /data-view="board"/);
