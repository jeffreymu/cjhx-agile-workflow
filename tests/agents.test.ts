...
test("trusted execution grants run once inside an isolated worktree", async (t) => {
  const { root, script, service } = fixture(t); const worktree = resolve(root, "worktree"); mkdirSync(worktree); const baseCommit = "a".repeat(40); const now = new Date().toISOString(); service.storage.saveCollaboration({ schemaVersion: 1, id: "collaboration-1", workspaceId: "workspace-1", changeId: "CHANGE-1", taskId: "task-1", title: "Grant", objective: "Run", status: "running", limits: { maxAssignments: 1, maxDepth: 0, maxParallel: 1, maxMessages: 1, timeoutMinutes: 5 }, usage: { assignments: 1, runningAssignments: 0, messages: 0, tokens: 0 }, createdBy: "owner", createdAt: now, updatedAt: now }); service.storage.saveAgentAssignment({ schemaVersion: 1, id: "assignment-1", collaborationId: "collaboration-1", depth: 0, workspaceId: "workspace-1", changeId: "CHANGE-1", taskId: "task-1", agentId: "pi", role: "implementer", objective: "Run", acceptanceCriteria: ["Done"], mode: "write", dependencyIds: [], status: "ready", worktreeLeaseId: "worktree-lease-1", proposedBy: { kind: "human", actor: "owner" }, createdAt: now }); service.storage.saveWorktreeLease({ schemaVersion: 1, id: "worktree-lease-1", collaborationId: "collaboration-1", assignmentId: "assignment-1", workspaceId: "workspace-1", repositoryRoot: root, path: worktree, branch: "cjhx/collaboration/assignment", baseRevision: "HEAD", baseCommit, status: "active", createdAt: new Date().toISOString() });
  ...
  assert.throws(() => service.issueExecutionGrant({ collaborationId: "../escape", assignmentId: "assignment-1", mode: "write", cwd: worktree }), /unsupported/);
  assert.throws(() => service.issueExecutionGrant({ collaborationId: "collaboration-1", assignmentId: "assignment-1", worktreeLeaseId: "worktree-lease-1", baseCommit, mode: "write", cwd: worktree, environment: { PATH: "unsafe" } }), /environment/);
  assert.throws(() => service.issueExecutionGrant({ collaborationId: "collaboration-1", assignmentId: "assignment-1", worktreeLeaseId: "worktree-lease-1", baseCommit: "b".repeat(40), mode: "write", cwd: worktree }), /does not match/);
  const executionGrantId = service.issueExecutionGrant({ collaborationId: "collaboration-1", assignmentId: "assignment-1", worktreeLeaseId: "worktree-lease-1", baseCommit, mode: "write", cwd: worktree, environment: { CJHX_COLLABORATION_ID: "collaboration-1" } });
...
test("Agent runs estimate tokens when a provider does not report usage and aggregate by Task and Change", async (t) => {
  const { script, service } = fixture(t); await service.save({ id: "codex", name: "Codex", kind: "codex", command: process.execPath, arguments: [script, "{prompt}"], versionArguments: [script, "--version"], promptTransport: "argument", timeoutMinutes: 5, environmentKeys: [] }, { approved: true }); const completed = await waitForRun(service, service.startTask("task-1", { approved: true }).id); assert.equal(completed.usage?.source, "estimated"); assert.ok((completed.usage?.inputTokens ?? 0) > 0); const summary = service.usageSummary({ kind: "task", id: "task-1" }); assert.equal(summary.runs, 1); assert.ok(summary.estimatedTokens > 0); assert.equal(summary.measuredTokens, 0); const changeSummary = service.usageSummary({ kind: "change", id: "CHANGE-1" }); assert.equal(changeSummary.runs, 1); assert.equal(changeSummary.totalTokens, summary.totalTokens); assert.equal(changeSummary.scope.kind, "change");
});

test("Agent usage aggregates all token dimensions by Change without cross-Change leakage", async (t) => {
  const { script, service } = fixture(t); await service.save({ id: "codex", name: "Codex", kind: "codex", command: process.execPath, arguments: [script, "{prompt}"], versionArguments: [script, "--version"], promptTransport: "argument", timeoutMinutes: 5, environmentKeys: [] }, { approved: true }); const completed = await waitForRun(service, service.startTask("task-1", { approved: true }).id); service.storage.saveAgentRun({ ...completed, usage: { source: "provider-reported", inputTokens: 120, outputTokens: 30, cacheReadTokens: 18, cacheWriteTokens: 7, reasoningTokens: 11, totalTokens: 150, observedAt: completed.completedAt ?? completed.startedAt } }); service.storage.saveAgentRun({ ...completed, id: "agent-run-other-change", changeId: "CHANGE-2", usage: { source: "provider-reported", inputTokens: 999, outputTokens: 1, cacheReadTokens: 500, totalTokens: 1_000, observedAt: completed.completedAt ?? completed.startedAt } }); const summary = service.usageSummary({ kind: "change", id: "CHANGE-1" }); assert.equal(summary.runs, 1); assert.equal(summary.inputTokens, 120); assert.equal(summary.outputTokens, 30); assert.equal(summary.cacheReadTokens, 18); assert.equal(summary.cacheWriteTokens, 7); assert.equal(summary.reasoningTokens, 11); assert.equal(summary.totalTokens, 150); assert.equal(summary.measuredTokens, 150);
});
