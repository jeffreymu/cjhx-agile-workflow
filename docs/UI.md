...
- compact left navigation led by Dashboard and first-class 目标, followed by Workspaces, delivery flow, Automation, integrations, Agents, Skills, and runs; the `<<` control collapses it to a persistent icon rail that retains the CJHX logo and bottom theme control, and `>>` restores the full menu;
