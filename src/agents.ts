...
import type { ComplianceReport, HarnessExecutionTarget, HarnessService, RuleSnapshot } from "./harness.js";
import type { JsonValue } from "./models.js";
import { utcNow } from "./models.js";
import type { Workspace } from "./storage.js";
import type { Task } from "./tasks.js";

export const agentKinds = ["pi", "claude-code", "codex", "qoder", "deepseek-harness", "custom"] as const;
...
export class DeepSeekHarnessUsageCollector implements AgentUsageCollector { collect(value: string): AgentTokenUsage | undefined { return parseAgentUsage(value, "deepseek-harness"); } }
...
export interface TokenUsageSummary { scope: { kind: "all" | "run" | "session" | "task" | "change" | "workspace" | "automation" | "automation-run"; id?: string }; runs: number; runningRuns: number; completedRuns: number; inputTokens: number; outputTokens: number; cacheReadTokens: number; cacheWriteTokens: number; reasoningTokens: number; totalTokens: number; measuredTokens: number; estimatedTokens: number; unavailableRuns: number; updatedAt: string }
...
export interface AgentExecutionGrantInput { collaborationId: string; assignmentId: string; worktreeLeaseId?: string; baseCommit?: string; cwd: string; environment?: Record<string, string>; prepareEnvironment?: (runId: string) => Record<string, string> }
interface StoredAgentExecutionGrant extends AgentExecutionGrantInput { id: string; createdAt: string }
...
export interface StartAgentTaskOptions { agentId?: string; instructions?: string; userMessage?: string; historicalContext?: string; approved: boolean; approvedRuleDigest?: string; context?: AgentRunContext; automationId?: string; automationRunId?: string; executionGrantId?: string; onCompleted?: (run: AgentRun) => void }
...
function usageCollector(kind: AgentKind): AgentUsageCollector { return kind === "pi" ? new PiUsageCollector() : kind === "claude-code" ? new ClaudeCodeUsageCollector() : kind === "codex" ? new CodexUsageCollector() : kind === "qoder" ? new QoderUsageCollector() : kind === "deepseek-harness" ? new DeepSeekHarnessUsageCollector() : new CustomAgentUsageCollector(); }
...
export class AgentService {
  ...
  private executionGrants = new Map<string, StoredAgentExecutionGrant>();
  ...
  prepareTask(taskId: string, options: { agentId?: string; instructions?: string; userMessage?: string; historicalContext?: string } = {}, harnessTarget?: HarnessExecutionTarget): AgentPreparedTask {
    ...
    const snapshot = this.dependencies.harness?.effectiveForTask(task.id, harnessTarget); const prompt = this.prompt(task, instructions, snapshot, historicalContext, userMessage); if (prompt.includes("\0")) throw new ValidationError("Agent prompt contains unsupported characters");
  ...
  startTask(taskId: string, options: StartAgentTaskOptions): AgentRun {
    ...
    const pendingGrant = options.executionGrantId ? this.peekExecutionGrant(options.executionGrantId) : undefined; const harnessTarget: HarnessExecutionTarget | undefined = pendingGrant?.worktreeLeaseId ? { kind: "worktree", rootPath: pendingGrant.cwd, worktreeLeaseId: pendingGrant.worktreeLeaseId, ...(pendingGrant.baseCommit ? { baseCommit: pendingGrant.baseCommit } : {}) } : undefined; const prepared = this.prepareTask(taskId, options, harnessTarget); const { task, profile, workspace, prompt, ruleSnapshot: snapshot } = prepared; const harness = this.dependencies.harness; const executor = this.dependencies.executor ?? new LocalProcessExecutor(); const executionGrant = options.executionGrantId ? this.consumeExecutionGrant(options.executionGrantId) : undefined;
    ...
    const id = `agent-run-${randomUUID().replaceAll("-", "")}`; const initialUsage: AgentTokenUsage = { source: "estimated", inputTokens: estimatedTokens(prompt), outputTokens: 0, totalTokens: estimatedTokens(prompt), observedAt: utcNow() }; const executionRoot = executionGrant?.cwd ?? workspace.rootPath; let preparedEnvironment: Record<string, string> = {}; try { preparedEnvironment = this.grantEnvironment(executionGrant?.prepareEnvironment?.(id)); } catch (error) { throw new PolicyDenied(`Agent execution grant environment preparation failed: ${error instanceof Error ? error.message : String(error)}`); } const run: AgentRun = { id, taskId: task.id, changeId: task.changeId, workspaceId: task.workspaceId!, agentId: profile.id, agentName: profile.name, agentKind: profile.kind, status: "running", startedAt: utcNow(), processInstanceId: this.processInstanceId, executionRoot, ...(executionGrant ? { collaborationId: executionGrant.collaborationId, assignmentId: executionGrant.assignmentId, ...(executionGrant.worktreeLeaseId ? { worktreeLeaseId: executionGrant.worktreeLeaseId } : {}) } : {}), ...(instructions ? { instructions } : {}), ...(snapshot?.sources.length ? { ruleSnapshotId: snapshot.id, ruleSnapshotDigest: snapshot.digest, complianceStatus: "pending" } : {}), ...(options.context ? options.context : {}), ...(options.automationId ? { automationId: options.automationId } : {}), ...(options.automationRunId ? { automationRunId: options.automationRunId } : {}), usage: initialUsage, usageUpdatedAt: initialUsage.observedAt, stdout: "", stderr: "" }; this.storage.saveAgentRun(run);
    let child: ChildProcessWithoutNullStreams; try { child = executor.start({ command: profile.command, arguments: args, cwd: executionRoot, environment: { ...this.environment(profile), ...(executionGrant?.environment ?? {}), ...preparedEnvironment }, ...(snapshot ? { ruleSnapshot: snapshot } : {}) }); } catch (error) { run.status = "failed"; run.exitCode = -1; run.completedAt = utcNow(); run.stderr = limited(run.stderr, Buffer.from(error instanceof Error ? error.message : String(error))); this.storage.saveAgentRun(run); queueMicrotask(() => { try { options.onCompleted?.(run); } catch { /* completion projection is best-effort; AgentRun remains authoritative */ } }); return run; }
  ...
  private peekExecutionGrant(id: string): StoredAgentExecutionGrant { const normalized = this.safeIdentifier(id, "Agent execution grant id"); const grant = this.executionGrants.get(normalized); if (!grant) throw new PolicyDenied("Agent execution grant is invalid or already consumed"); return grant; }
  private consumeExecutionGrant(id: string): StoredAgentExecutionGrant { const grant = this.peekExecutionGrant(id); this.executionGrants.delete(grant.id); return grant; }
  private grantEnvironment(value?: Record<string, string>): Record<string, string> { return Object.fromEntries(Object.entries(value ?? {}).map(([key, item]) => { if (!/^CJHX_[A-Z0-9_]+$/.test(key) || typeof item !== "string" || item.includes("\0") || Buffer.byteLength(item) > 4_096) throw new ValidationError("invalid Agent execution grant environment"); return [key, item]; })); }
  private safeIdentifier(value: string, label: string): string { const normalized = value.trim(); if (!identifier.test(normalized)) throw new ValidationError(`${label} contains unsupported characters`); return normalized; }
