...
export interface AgentExecutionGrantInput { collaborationId: string; assignmentId: string; worktreeLeaseId?: string; cwd: string; environment?: Record<string, string>; prepareEnvironment?: (runId: string) => Record<string, string> }
...
export class AgentService {
  ...
  startTask(taskId: string, options: StartAgentTaskOptions): AgentRun {
    ...
    const prepared = this.prepareTask(taskId, options); const { task, profile, workspace, prompt, ruleSnapshot: snapshot } = prepared; const harness = this.dependencies.harness; const executor = this.dependencies.executor ?? new LocalProcessExecutor(); const executionGrant = options.executionGrantId ? this.consumeExecutionGrant(options.executionGrantId) : undefined;
  ...
  private consumeExecutionGrant(id: string): StoredAgentExecutionGrant { const normalized = this.safeIdentifier(id, "Agent execution grant id"); const grant = this.executionGrants.get(normalized); if (!grant) throw new PolicyDenied("Agent execution grant is invalid or already consumed"); this.executionGrants.delete(normalized); return grant; }
