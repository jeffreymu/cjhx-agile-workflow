...
export interface StartAgentTaskOptions { agentId?: string; instructions?: string; userMessage?: string; historicalContext?: string; approved: boolean; approvedRuleDigest?: string; context?: AgentRunContext; onCompleted?: (run: AgentRun) => void }
...
export class AgentService {
  ...
  startTask(taskId: string, options: StartAgentTaskOptions): AgentRun {
    ...
    const id = `agent-run-${randomUUID().replaceAll("-", "")}`; const run: AgentRun = { id, taskId: task.id, changeId: task.changeId, workspaceId: task.workspaceId!, agentId: profile.id, agentName: profile.name, agentKind: profile.kind, status: "running", startedAt: utcNow(), processInstanceId: this.processInstanceId, ...(instructions ? { instructions } : {}), ...(snapshot?.sources.length ? { ruleSnapshotId: snapshot.id, ruleSnapshotDigest: snapshot.digest, complianceStatus: "pending" } : {}), ...(options.context ? options.context : {}), stdout: "", stderr: "" }; this.storage.saveAgentRun(run);
    ...
    child.stdout.on("data", (chunk: Buffer) => { if (!this.ownsRun(run)) return; run.stdout = limited(run.stdout, chunk); this.storage.saveAgentRun(run); }); child.stderr.on("data", (chunk: Buffer) => { if (!this.ownsRun(run)) return; run.stderr = limited(run.stderr, chunk); this.storage.saveAgentRun(run); });
