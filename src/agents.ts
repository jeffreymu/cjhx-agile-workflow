...
interface AgentDependencies { task(id: string): Task; workspace(id: string): AgentWorkspace; enabledSkills?(): { name: string; description: string; path: string }[] }
...
export class AgentService {
  ...
  startTask(taskId: string, options: { agentId?: string; instructions?: string; approved: boolean }): AgentRun {
    ...
    const instructions = options.instructions?.trim(); const prompt = this.prompt(task, instructions); const args = profile.arguments.map((argument) => argument.replaceAll("{prompt}", () => prompt).replaceAll("{taskId}", () => task.id).replaceAll("{changeId}", () => task.changeId));
    const id = `agent-run-${randomUUID().replaceAll("-", "")}`; const run: AgentRun = { id, taskId: task.id, changeId: task.changeId, workspaceId: task.workspaceId, agentId: profile.id, agentName: profile.name, status: "running", startedAt: utcNow(), ...(instructions ? { instructions } : {}), stdout: "", stderr: "" }; this.storage.saveAgentRun(run);
    const child = spawn(profile.command, args, { cwd: workspace.rootPath, env: this.environment(profile), stdio: ["pipe", "pipe", "pipe"], shell: false }); this.children.set(id, child);
    ...
    let timedOut = false; let forceTimer: NodeJS.Timeout | undefined; const timer = setTimeout(() => { timedOut = true; child.kill("SIGTERM"); forceTimer = setTimeout(() => child.kill("SIGKILL"), 2_000); forceTimer.unref(); }, profile.timeoutMinutes * 60_000); timer.unref();
    child.on("error", (error) => { clearTimeout(timer); if (forceTimer) clearTimeout(forceTimer); run.status = timedOut ? "timed_out" : "failed"; run.stderr = limited(run.stderr, Buffer.from(error.message)); run.completedAt = utcNow(); this.storage.saveAgentRun(run); this.children.delete(id); });
    child.on("close", (code) => { clearTimeout(timer); if (forceTimer) clearTimeout(forceTimer); run.status = timedOut ? "timed_out" : code === 0 ? "succeeded" : "failed"; run.exitCode = code ?? -1; run.completedAt = utcNow(); this.storage.saveAgentRun(run); this.children.delete(id); });
    return run;
  }

  private prompt(task: Task, instructions?: string): string { const skills = this.dependencies.enabledSkills?.() ?? []; const skillContext = skills.length ? `Enabled local CJHX Agent skills:\n${skills.map((skill) => `- ${skill.name}: ${skill.description}\n  Read and follow: ${skill.path}`).join("\n")}\nUse only the skills relevant to this task.` : ""; return [`Implement CJHX task ${task.id} for change ${task.changeId}.`, `Title: ${task.title}`, task.description ? `Description: ${task.description}` : "", task.acceptanceCriteria.length ? `Acceptance criteria:\n${task.acceptanceCriteria.map((item) => `- ${item}`).join("\n")}` : "", skillContext, instructions ? `Additional instructions:\n${instructions}` : "", "Create or modify files only inside the current repository. You may read only the explicitly listed Skill instruction files outside it. Do not push, publish, deploy, or perform destructive Git operations. Summarize changes and validation when complete."].filter(Boolean).join("\n\n"); }
