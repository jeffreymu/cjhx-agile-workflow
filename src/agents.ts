...
interface AgentDependencies { task(id: string): Task; workspace(id: string): AgentWorkspace }
...
export class AgentService {
  ...
  private prompt(task: Task, instructions?: string): string { return [`Implement CJHX task ${task.id} for change ${task.changeId}.`, `Title: ${task.title}`, task.description ? `Description: ${task.description}` : "", task.acceptanceCriteria.length ? `Acceptance criteria:\n${task.acceptanceCriteria.map((item) => `- ${item}`).join("\n")}` : "", instructions ? `Additional instructions:\n${instructions}` : "", "Work only inside the current repository. Do not push, publish, deploy, or perform destructive Git operations. Summarize changes and validation when complete."].filter(Boolean).join("\n\n"); }
