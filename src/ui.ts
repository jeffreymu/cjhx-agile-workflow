...
function snapshot(app: CJHXFramework): JsonObject {
  ...
  const agentRuns = app.agents.listRuns().map(({ id, taskId, changeId, workspaceId, agentId, agentName, status, startedAt, completedAt, exitCode, ruleSnapshotDigest, complianceStatus, complianceReportId }) => ({ id, taskId, changeId, workspaceId, agentId, agentName, status, startedAt, ...(completedAt ? { completedAt } : {}), ...(exitCode !== undefined ? { exitCode } : {}), ...(ruleSnapshotDigest ? { ruleSnapshotDigest } : {}), ...(complianceStatus ? { complianceStatus } : {}), ...(complianceReportId ? { complianceReportId } : {}) }));
