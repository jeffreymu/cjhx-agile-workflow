...
import { persistableErrorMessage, SkillRuntime, type SkillOutputPolicy } from "./skills.js";
import { Workspace } from "./storage.js";

export interface WorkflowStep { id: string; skill: string; input: JsonObject; pinned?: { version: string; digest: string } }
...
export function parseWorkflowDefinition(value: unknown): WorkflowDefinition {
  ...
  const seen = new Set<string>(); const steps = value.steps.map((raw): WorkflowStep => {
    ...
    const pinned = raw.pinned; if (pinned !== undefined && (!isRecord(pinned) || typeof pinned.version !== "string" || typeof pinned.digest !== "string")) throw new ValidationError("workflow pinned Skill requires version and digest");
    if (seen.has(raw.id)) throw new ValidationError(`duplicate workflow step id: ${raw.id}`); seen.add(raw.id); return { id: raw.id, skill: raw.skill, input: raw.input as JsonObject, ...(pinned ? { pinned: { version: pinned.version as string, digest: pinned.digest as string } } : {}) };
  });
...
export class WorkflowRuntime {
  ...
  async run(definition: WorkflowDefinition, payload: JsonObject, options: { changeId?: string; workspaceId?: string; approvedSteps?: Set<string>; auditInput?: JsonObject; outputPolicy?: SkillOutputPolicy } = {}): Promise<WorkflowRun> {
    ...
    try {
      for (const step of definition.steps) {
        const resolved = this.resolve(step.input, context) as JsonObject; const run = await this.skills.run(step.skill, resolved, { ...(options.changeId ? { changeId: options.changeId } : {}), ...(options.workspaceId ? { workspaceId: options.workspaceId } : {}), approved: options.approvedSteps?.has(step.id) ?? false, ...(step.pinned ? { pinned: step.pinned } : {}), ...(options.auditInput ? { auditInput: { workbench: options.auditInput, stepId: step.id } } : {}), ...(options.outputPolicy ? { outputPolicy: options.outputPolicy } : {}) });
      ...
      const run: WorkflowRun = { id, workflowId: definition.id, workflowVersion: definition.version, ...(options.changeId ? { changeId: options.changeId } : {}), ...(options.workspaceId ? { workspaceId: options.workspaceId } : {}), status: "succeeded", startedAt, completedAt: utcNow(), input: redact(options.auditInput ?? payload) as JsonObject, steps: redact(records) as JsonObject[], output: redact(((context.steps as JsonObject)[last.id] as JsonObject).output ?? {}) as JsonObject };
      this.workspace.saveRun(run); return run;
    } catch (error) {
      const run: WorkflowRun = { id, workflowId: definition.id, workflowVersion: definition.version, ...(options.changeId ? { changeId: options.changeId } : {}), ...(options.workspaceId ? { workspaceId: options.workspaceId } : {}), status: "failed", startedAt, completedAt: utcNow(), input: redact(options.auditInput ?? payload) as JsonObject, steps: redact(records) as JsonObject[], output: {}, error: persistableErrorMessage(error, options.outputPolicy) };
