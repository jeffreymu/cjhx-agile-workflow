...
export class InMemoryDevOpsAdapter implements DevOpsAdapter {
  ...
  private filtered(items: Map<string, JsonObject>, request: JsonObject): JsonObject[] { const changeId = typeof request.changeId === "string" ? request.changeId : undefined; return [...items.values()].filter((item) => !changeId || item.changeId === changeId).map((item) => ({ ...item })); }
}
