...
export class HarnessService {
  ...
  effectiveForTask(taskId: string, target?: HarnessExecutionTarget): RuleSnapshot {
    ...
    const effective = this.compile(bundles); const sourceRecords = bundles.map((bundle) => ({ id: bundle.id, version: bundle.version, scope: bundle.scope, mode: bundle.mode, digest: hash(bundle) })); const executionTarget = target ? { kind: target.kind, ...(target.worktreeLeaseId ? { worktreeLeaseId: target.worktreeLeaseId } : {}), ...(target.baseCommit ? { baseCommit: target.baseCommit } : {}) } : undefined; const snapshotContext = { workspaceId: context.workspaceId, changeId: context.changeId, taskId: context.taskId, ...(executionTarget ? { executionTarget } : {}) }; const digest = hash({ context: snapshotContext, sources: sourceRecords, effective }); const snapshot: RuleSnapshot = { id: `rule-snapshot-${randomUUID().replaceAll("-", "")}`, digest, compiledAt: utcNow(), context: snapshotContext, sources: sourceRecords, effective }; this.storage.saveRuleSnapshot(snapshot); return snapshot;
  }
