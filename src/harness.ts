...
export interface RuleSnapshot {
  ...
  digest: string;
  compiledAt: string;
  context: Omit<HarnessContext, "workspaceRoot">;
...
export interface ComplianceReport { id: string; runId: string; taskId: string; changeId: string; workspaceId: string; ruleSnapshotId: string; ruleSnapshotDigest: string; repositoryStateDigest: string; agentRunStatus: "succeeded" | "failed" | "timed_out"; status: "passed" | "failed"; checks: HarnessCheckResult[]; createdAt: string }
...
export class HarnessService {
  ...
  effectiveForTask(taskId: string): RuleSnapshot {
    ...
    const effective = this.compile(bundles); const sourceRecords = bundles.map((bundle) => ({ id: bundle.id, version: bundle.version, scope: bundle.scope, mode: bundle.mode, digest: hash(bundle) })); const snapshotContext = { workspaceId: context.workspaceId, changeId: context.changeId, taskId: context.taskId }; const digest = hash({ context: snapshotContext, sources: sourceRecords, effective }); const snapshot: RuleSnapshot = { id: `rule-snapshot-${randomUUID().replaceAll("-", "")}`, digest, compiledAt: utcNow(), context: snapshotContext, sources: sourceRecords, effective }; this.storage.saveRuleSnapshot(snapshot); return snapshot;
  }
  ...
  private repositoryStateDigest(cwd: string): string { try { const root = realpathSync(cwd); const git = (arguments_: string[]): Buffer => execFileSync("git", arguments_, { cwd: root, encoding: "buffer", maxBuffer: 32 * 1_048_576, stdio: ["ignore", "pipe", "ignore"] }); let head: Buffer; try { head = git(["rev-parse", "--verify", "HEAD"]); } catch { head = Buffer.from("UNBORN-HEAD\n"); } const working = git(["diff", "--binary", "--no-ext-diff"]); const staged = git(["diff", "--cached", "--binary", "--no-ext-diff"]); const untracked = git(["ls-files", "--others", "--exclude-standard", "-z"]).toString("utf8").split("\0").filter(Boolean).sort(); const digest = createHash("sha256").update(head).update(working).update(staged); let total = head.length + working.length + staged.length; for (const relativePath of untracked) { if (relativePath.startsWith("/") || relativePath.split("/").includes("..")) throw new ValidationError("Git returned an unsafe untracked path"); const path = resolve(root, relativePath); const stat = lstatSync(path); if (!stat.isFile() && !stat.isSymbolicLink()) throw new ValidationError("Harness repository state contains an unsupported file type"); const expectedSize = stat.isSymbolicLink() ? Buffer.byteLength(readlinkSync(path)) : stat.size; total += Buffer.byteLength(relativePath) + expectedSize; if (total > 32 * 1_048_576) throw new ValidationError("Harness repository state exceeds 32 MB"); const content = stat.isSymbolicLink() ? Buffer.from(readlinkSync(path)) : readFileSync(path); digest.update(relativePath).update("\0").update(content).update("\0"); } return `sha256:${digest.digest("hex")}`; } catch (error) { if (error instanceof ValidationError) throw error; throw new ValidationError(`cannot fingerprint Harness repository state: ${error instanceof Error ? error.message.split("\n")[0] : String(error)}`); } }
