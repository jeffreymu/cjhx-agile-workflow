...
export class Workspace {
  ...
  saveCollaborationPlanSnapshot(value: CollaborationPlanSnapshot): void { this.initialize(); this.createPrivateEntity(this.collaborationPlanSnapshots, value.id, "collaboration plan snapshot id", value as unknown as JsonValue, "collaboration plan snapshot already exists"); }
  ...
  saveAgentMessage(value: AgentMessage): void { this.initialize(); const path = this.entityPath(this.collaborationMessages, value.id, "agent message id"); const existing = this.readPrivateJson(path) as unknown as AgentMessage; const immutable = (message: AgentMessage) => { const { status: _status, consumedAt: _consumedAt, ...rest } = message; return rest; }; if (JSON.stringify(immutable(existing)) !== JSON.stringify(immutable(value))) throw new ValidationError("agent message immutable content cannot be changed"); this.writePrivateJson(path, value as unknown as JsonValue); }
