...
export class HttpDevOpsAdapter implements DevOpsAdapter {
  ...
  private async list(path: string, request: JsonObject): Promise<JsonObject[]> { const query = new URLSearchParams(); const changeId = optional(request.changeId); const workspaceId = optional(request.workspaceId); if (changeId) query.set("changeId", changeId); if (workspaceId) query.set("workspaceId", workspaceId); const value = await this.value(`${path}${query.size ? `?${query}` : ""}`, { method: "GET" }); if (!Array.isArray(value) || !value.every(isRecord)) throw new AdapterError("DevOps Gateway returned an invalid list"); return value as JsonObject[]; }
