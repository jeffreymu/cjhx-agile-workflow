...
export interface ProvisionWorktreeInput { collaborationId: string; assignmentId: string; workspaceId: string; baseRevision: string; approved: boolean }
...
export class WorktreeLeaseService {
  ...
  provision(input: ProvisionWorktreeInput): WorktreeLease {
    ...
    const repositoryRoot = realpathSync(workspace.rootPath); const baseRevision = revision(input.baseRevision); const baseCommit = this.git(repositoryRoot, ["rev-parse", "--verify", `${baseRevision}^{commit}`]).trim();
  ...
  remove(id: string, options: { approved: boolean }): WorktreeLease {
    if (!options.approved) throw new PolicyDenied("worktree lease removal requires human approval"); const lease = this.get(id); if (lease.status === "removed") return lease; if (!existsSync(lease.path)) { lease.status = "removed"; lease.removedAt = new Date().toISOString(); this.storage.saveWorktreeLease(lease); return lease; }
    ...
    const dirty = this.git(actual, ["status", "--porcelain=v1"], true).trim(); if (dirty) { lease.status = "cleanup_pending"; this.storage.saveWorktreeLease(lease); throw new PolicyDenied("worktree lease has uncommitted changes and cannot be removed"); }
  ...
  private git(root: string, args: string[], allowFailure = false): string { try { return execFileSync("git", ["-C", root, ...args], { encoding: "utf8", timeout: 10_000, maxBuffer: 1_048_576, stdio: ["ignore", "pipe", allowFailure ? "ignore" : "pipe"] }); } catch (error) { if (allowFailure) return ""; throw new ValidationError(`Git operation failed: ${error instanceof Error ? error.message.split("\n")[0] : String(error)}`); } }
}
