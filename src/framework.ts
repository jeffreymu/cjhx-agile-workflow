...
import { CollaborationBridge, CollaborationMessageService } from "./collaboration-bridge.js";
import { CollaborationScheduler } from "./collaboration-scheduler.js";
import { CollaborationService, type CollaborationPlanInput, type CreateCollaborationInput, type HumanAssignmentInput } from "./collaboration-service.js";
...
import { TestWorkbenchService } from "./test-workbench.js";
import { WorkflowRuntime, type WorkflowDefinition, type WorkflowRun } from "./workflows.js";
import { WorktreeLeaseService } from "./worktree-leases.js";
import { WorkspaceHub } from "./workspace-hub.js";

export class CJHXFramework {
  readonly workspace: Workspace; readonly policy: Policy; readonly registry: SkillRegistry; readonly localSkills: LocalSkillService; readonly runtime: SkillRuntime; readonly workflows: WorkflowRuntime; readonly testWorkbench: TestWorkbenchService; readonly tasks: TaskService; readonly devops: DevOpsService; readonly devopsIntegration: DevOpsIntegrationManager; readonly jiraIntegration: JiraIntegrationManager; readonly sourceControlIntegration: SourceControlIntegrationManager; readonly gitLabIntegration: GitLabIntegrationManager; readonly gitHubIntegration: GitHubIntegrationManager; readonly workspaceHub: WorkspaceHub; readonly harness: HarnessService; readonly agents: AgentService; readonly memory: MemoryService; readonly conversations: ConversationService; readonly collaborationMessages: CollaborationMessageService; readonly collaborations: CollaborationService; readonly worktreeLeases: WorktreeLeaseService; readonly collaborationBridge: CollaborationBridge; readonly automations: AutomationService; readonly goals: GoalService; readonly dashboard: DashboardService; private collaborationScheduler?: CollaborationScheduler;
  constructor(workspace = ".cjhx", options: { policy?: Policy; tools?: ToolBroker; localSkillRoots?: string[]; harnessRuleSources?: HarnessRuleSource[]; agentExecutor?: AgentExecutor } = {}) {
    this.workspace = new Workspace(workspace); this.policy = options.policy ?? new Policy(); this.registry = new SkillRegistry(this.workspace, this.policy); this.localSkills = new LocalSkillService(this.workspace, this.registry, this.policy, options.localSkillRoots); this.runtime = new SkillRuntime(this.workspace, this.registry, this.policy, options.tools); this.workflows = new WorkflowRuntime(this.workspace, this.runtime); this.tasks = new TaskService(this.workspace, this.runtime.tools); this.devops = new DevOpsService(this.runtime.tools); this.devopsIntegration = new DevOpsIntegrationManager(this.workspace, this.runtime.tools); this.jiraIntegration = new JiraIntegrationManager(this.workspace, this.runtime.tools); this.sourceControlIntegration = new SourceControlIntegrationManager(this.workspace, this.runtime.tools); this.gitLabIntegration = new GitLabIntegrationManager(this.workspace, this.sourceControlIntegration); this.gitHubIntegration = new GitHubIntegrationManager(this.workspace, this.sourceControlIntegration); this.workspaceHub = new WorkspaceHub(this.workspace, { remote: (provider) => provider === "gitlab" ? this.gitLabIntegration.browser() : this.gitHubIntegration.browser(), changes: () => this.workspace.listChanges(), tasks: () => this.tasks.list(), runs: () => [...this.workspace.listRuns(), ...this.workspace.listAgentRuns() as unknown as JsonObject[]], sessions: () => this.conversations.listSessions() as unknown as JsonObject[] }); this.testWorkbench = new TestWorkbenchService(this.workspace, this.registry, this.runtime, this.workflows, this.runtime.tools, { workspace: (id) => this.workspaceHub.get(id) }); this.harness = new HarnessService(this.workspace, { task: (id) => this.tasks.get(id), workspace: (id) => this.workspaceHub.get(id) }, options.harnessRuleSources); this.tasks.setTransitionGate((taskId, target) => this.harness.assertTaskGate(taskId, target)); this.agents = new AgentService(this.workspace, { task: (id) => this.tasks.get(id), workspace: (id) => this.workspaceHub.get(id), enabledSkills: () => this.localSkills.enabledAgentSkills(), harness: this.harness, ...(options.agentExecutor ? { executor: options.agentExecutor } : {}) }); this.memory = new MemoryService(this.workspace, { task: (id) => this.tasks.get(id), change: (id) => this.workspace.getChange(id), workspace: (id) => this.workspaceHub.get(id) }); this.conversations = new ConversationService(this.workspace, { task: (id) => this.tasks.get(id), agents: this.agents, memory: this.memory }); this.collaborationMessages = new CollaborationMessageService(this.workspace); this.worktreeLeases = new WorktreeLeaseService(this.workspace, { workspace: (id) => this.workspaceHub.get(id) }); this.collaborations = new CollaborationService(this.workspace, { task: (id) => this.tasks.get(id), workspace: (id) => this.workspaceHub.get(id), agentIds: () => this.agents.summary().profiles.map((profile) => profile.id), harnessSnapshotDigest: (taskId) => this.harness.effectiveForTask(taskId).digest, resolveBaseCommit: (workspaceId, revision) => this.worktreeLeases.resolveBaseCommit(workspaceId, revision) }); this.collaborationBridge = new CollaborationBridge(this.collaborationMessages, { delegate: (context, input) => { const assignment = this.collaborations.delegate(context, input); queueMicrotask(() => this.collaborationScheduler?.schedule(assignment.collaborationId)); return assignment; } }); this.automations = new AutomationService(this.workspace, { workspaces: this.workspaceHub, tasks: () => this.tasks.list(), devops: this.devops, harness: this.harness, agents: this.agents }); this.goals = new GoalService(this.workspace, { workspace: (id) => this.workspaceHub.get(id), changes: () => this.workspace.listChanges(), tasks: () => this.tasks.list(), findings: () => this.automations.listFindings() }); this.dashboard = new DashboardService({ goals: () => this.goals.portfolio(), changes: () => this.workspace.listChanges(), tasks: () => this.tasks.list(), agentRuns: () => this.agents.listRuns(), automationRuns: () => this.automations.listRuns(), automationReports: () => this.automations.listReports().map((report) => { try { return this.automations.reportView(report.id); } catch { return { ...report, stale: true }; } }), findings: () => this.automations.listFindings() });
  }
  initialize(): void { this.workspace.initialize(); }
  async startCollaborationBridge(): Promise<{ host: "127.0.0.1"; port: number; url: string }> { const address = await this.collaborationBridge.listen(); this.collaborationScheduler = new CollaborationScheduler(this.workspace, this.collaborations, this.agents, this.collaborationMessages, this.worktreeLeases, { bridgeUrl: address.url }); return address; }
  async closeCollaborationBridge(): Promise<void> { const scheduler = this.collaborationScheduler; this.collaborationScheduler = undefined; scheduler?.close(); await this.collaborationBridge.close(); }
  createCollaboration(input: CreateCollaborationInput) { return this.collaborations.create(input); }
  previewCollaborationPlan(id: string, input: CollaborationPlanInput) { return this.collaborations.previewPlan(id, input); }
  startCollaboration(id: string, input: { approved: boolean; approvedPlanDigest: string }) { const scheduler = this.requireCollaborationScheduler(); const collaboration = this.collaborations.start(id, input); scheduler.schedule(id); return collaboration; }
  addCollaborationAssignment(id: string, input: HumanAssignmentInput) { const scheduler = this.requireCollaborationScheduler(); const assignment = this.collaborations.addAssignment(id, input); scheduler.schedule(id); return assignment; }
  scheduleCollaboration(id: string) { return this.requireCollaborationScheduler().schedule(id); }
  cancelCollaboration(id: string, options: { approved: boolean }) { return this.requireCollaborationScheduler().cancel(id, options); }
  private requireCollaborationScheduler(): CollaborationScheduler { if (!this.collaborationScheduler) throw new ValidationError("Collaboration Bridge is not running"); return this.collaborationScheduler; }
