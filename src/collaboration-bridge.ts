...
import { collaborationPlanDigest, type AgentAssignment, type AgentMessage, type AgentMessageType, type CollaborationCapability } from "./collaboration.js";
...
export class CollaborationMessageService {
  ...
  issueCapability(input: { assignmentId: string; runId: string; permissions: CollaborationPermission[]; ttlSeconds?: number }): IssuedCollaborationCapability {
    ...
    if (assignment.agentRunId !== runId || assignment.status !== "running" || !assignment.worktreeLeaseId) throw new PolicyDenied("collaboration capability requires the Assignment's active Agent Run and Worktree Lease"); const run = this.storage.getAgentRun(runId); const collaboration = this.storage.getCollaboration(assignment.collaborationId); const lease = this.storage.getWorktreeLease(assignment.worktreeLeaseId); if (collaboration.status !== "running" || run.status !== "running" || run.collaborationId !== collaboration.id || run.assignmentId !== assignment.id || run.worktreeLeaseId !== lease.id || lease.status !== "active" || lease.assignmentId !== assignment.id || lease.collaborationId !== collaboration.id) throw new PolicyDenied("collaboration capability requires authoritative active Run, Collaboration, and Worktree state");
  ...
  authenticate(token: string, permission: CollaborationPermission): CapabilityContext {
    ...
    const assignment = this.storage.getAgentAssignment(capability.assignmentId); if (!assignment.worktreeLeaseId) throw new PolicyDenied("collaboration capability no longer matches an active Assignment"); const run = this.storage.getAgentRun(capability.runId); const collaboration = this.storage.getCollaboration(capability.collaborationId); const lease = this.storage.getWorktreeLease(assignment.worktreeLeaseId); if (collaboration.status !== "running" || assignment.collaborationId !== capability.collaborationId || assignment.agentId !== capability.agentId || assignment.agentRunId !== capability.runId || assignment.status !== "running" || run.status !== "running" || run.collaborationId !== collaboration.id || run.assignmentId !== assignment.id || run.worktreeLeaseId !== lease.id || lease.status !== "active" || lease.assignmentId !== assignment.id || lease.collaborationId !== collaboration.id) throw new PolicyDenied("collaboration capability no longer matches authoritative active Run, Collaboration, and Worktree state");
  ...
  send(token: string, raw: SendAgentMessageInput): AgentMessage {
    ...
    const collaboration = this.storage.getCollaboration(context.assignment.collaborationId); if (!collaboration.planSnapshotId || !collaboration.planDigest) throw new PolicyDenied("Collaboration message requires an approved plan"); const plan = this.storage.getCollaborationPlanSnapshot(collaboration.planSnapshotId); if (plan.collaborationId !== collaboration.id || plan.digest !== collaboration.planDigest || collaborationPlanDigest(plan) !== plan.digest) throw new PolicyDenied("Collaboration plan digest no longer matches"); const currentMessages = this.storage.listAgentMessages().filter((message) => message.collaborationId === collaboration.id).length; if (currentMessages >= plan.limits.maxMessages) throw new PolicyDenied("Collaboration message limit reached");
